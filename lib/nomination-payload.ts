  ...
  try {
    ...
    return (
      members.find((member) =>
        ...
        normalize(member.user?.global_name) === wantedDisplay ||
        normalize(member.user?.username) === wantedQuery ||
        normalize(member.nick) === wantedQuery ||
...
export async function buildNominationsPayload(includeArchive = false, options: BuildOptions = {}) {
  const cacheable = !options.skipRedis && !options.skipR2 && !options.skipDiscord;
  if (cacheable && nominationsCache && nominationsCache.expires > Date.now()) {
  ...
  const nominees = await Promise.all(nominationSeed.map(async (seed) => {
    ...
    return {
      ...
      source: {
        ...
        liveDiscordApi: Boolean(discordMember),
  ...
  if (cacheable) nominationsCache = { expires: Date.now() + 60 * 1000, payload };
