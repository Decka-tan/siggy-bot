...
export function computeStats(days: number, roleRank: number, rarity: string) {
  ...
  const atk = round20(Math.floor(sd       * mult * 12));
  const def = round20(Math.floor(roleRank * mult * 80));
  const spd = round20(Math.floor(sd       * roleRank * mult * 4));

  // Floor = what a 3-month member at roleRank 1 would get — derived from the same formula
  const minAtk = round20(Math.floor(SD_90 * mult * 12));
  const minDef = round20(Math.floor(1     * mult * 80));
  const minSpd = round20(Math.floor(SD_90 * 1    * mult * 4));
