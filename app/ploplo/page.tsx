...
// Accent for this badge page (Genesis uses gold — PloPlo gets Ritual green).
const ACCENT = '#40FFAF';
const BADGE_SLUG = 'ploplo-holder';
const BADGE_LABEL = 'PloPlo Holder';

function displayRoleOf(h: Holder): string | null {
  return h.topRole || h.fallbackRole || null;
}

// Bucket the holder by their CONTRIBUTOR-ladder role (or null = 'Other'), so a
// Mage+ritty member lands in the 'ritty' bucket even though the card still
// shows 'Mage'.
...
function ResultOverlay({
  ...
  const color = role ? ROLE_COLOR[role] || ACCENT : ACCENT;
  const line = useMemo(
              ...
              <div className="md:w-3/5 flex flex-col justify-center px-8 py-10">
                ...
                {(() => {
                  ...
                  return (
                    <p className="text-sm leading-relaxed mb-4" style={{ color: '#888' }}>
                      Locked in for <span className="font-bold" style={{ color }}>{formatDays(d)}</span> since joining Ritual. Holds the {BADGE_LABEL} badge. 🔥
                    </p>
                  );
                })()}
                ...
                {activity && ((activity.contributions ?? 0) || (activity.eventsWon ?? 0) || (activity.eventsHosted ?? 0)) ? (
                  <div className="grid grid-cols-3 gap-2">
                    ...
                    <ActivityTile label="Won" value={activity.eventsWon ?? 0} accent="#FFD700" />
            ...
            <div className="relative min-h-[400px] flex flex-col md:flex-row">
              ...
              <div className="md:w-3/5 flex flex-col justify-center px-8 py-10">
                ...
                <p className="text-sm leading-relaxed mb-2" style={{ color: '#aaa' }}>
                  This member doesn&apos;t hold the {BADGE_LABEL} badge.
                </p>
                <p className="text-sm leading-relaxed" style={{ color: '#777' }}>
                  Keep showing up in the community — badges get handed out to the ones who do. 🌱
                </p>
              </div>
            </div>
          )}
...
function MemberCard({ holder, onClick }: { holder: Holder; onClick: () => void }) {
  ...
  return (
    <div
      onClick={onClick}
      className="ploplo-card group relative flex flex-col rounded-xl border overflow-hidden transition-all duration-300 hover:-translate-y-1 cursor-pointer"
      style={{
        backgroundColor: '#0a0a0a',
...
export default function PloPloPage() {
  ...
  // Deep-link: /ploplo?u=username opens modal on load.
  useEffect(() => {
    if (!data) return;
  ...
  // Cursed, Harmonic, no-role) collapse into a single "Other" bucket.
      ...
      <section ref={heroRef} className="relative min-h-screen flex flex-col items-center justify-center px-6 text-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto">
          <p className="hero-line opacity-0 font-mono text-xs tracking-[0.3em] uppercase mb-6" style={{ color: '#666' }}>
            {BADGE_LABEL} · Community Registry
          </p>
          ...
          <p className="hero-line opacity-0 text-lg md:text-xl mb-10" style={{ color: '#888' }}>
            {data ? `${data.count} members` : 'Loading…'} holding the {BADGE_LABEL} badge
          </p>
          ...
          <>
            ...
            <div className="flex flex-wrap justify-center gap-4 mb-14">
              {mainPresent.map(role => {
                ...
                const color = ROLE_COLOR[role] || ACCENT;
                return (
                  <div
                    key={role}
            ...
            <div className="flex flex-wrap gap-2 mb-10 justify-center">
              {(['all', ...mainPresent, ...(otherCount > 0 ? ['other'] : [])] as string[]).map(role => {
                ...
                const color = role === 'all' ? '#fff' : role === 'other' ? '#888' : ROLE_COLOR[role] || ACCENT;
                const count =
                  role === 'all'
                    ? data.holders.length
                    : role === 'other'
