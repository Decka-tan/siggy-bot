...
function buildDashboardData(selectedGuildId?: string, filters?: { q?: string; status?: string; creator?: string }): DashboardData {
  ...
  let invoices = Object.values(db.invoices || {}).sort((a, b) => b.createdAt - a.createdAt);
  ...
  // Apply Search/Logic Filters
  const filteredInvoices = invoices.filter(inv => {
    // Search Query (Title or Participant Name)
    if (filters?.q) {
  ...
  return {
    ...
    stats: {
      ...
      paidParticipants: allParticipants.filter((p) => p.paid).length,
  ...
  return (
    <div className="relative overflow-hidden rounded-lg border-l-4 border-accent bg-[#2b2d31] p-4 shadow-md transition-all hover:bg-[#313338]">
          ...
          <div key={idx} className="flex items-center justify-between py-1 border-b border-white/5 last:border-0">
            ...
            <div className="flex items-center gap-3">
              ...
              <form action={participantAction.bind(null, invoice.id, idx, !p.paid)}>
                <button type="submit" className={`flex h-6 w-6 items-center justify-center rounded transition-all ${p.paid ? "bg-emerald-500/10 text-emerald-400" : "bg-white/5 text-[#b5bac1] hover:text-amber-400"}`}>
      ...
      <div className="mt-4 flex items-center justify-end gap-2">
        <button className="flex items-center gap-1.5 rounded bg-[#4e5058] px-3 py-1.5 text-[10px] font-medium text-white transition-all">
...
export default async function InvoiceDashboardPage({ searchParams }: { searchParams: { guild?: string; tab?: string; error?: string; q?: string; status?: string; creator?: string } }) {
  ...
  if (!isAuth) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#0a0a0a] p-6 text-text-primary">
        ...
        <div className="w-full max-w-md space-y-8 rounded-3xl border border-white/5 bg-[#0d0d0d] p-10 shadow-2xl">
          <div className="text-center">
            <div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-accent to-yellow-500">
              <Lock className="h-8 w-8 text-black" />
            </div>
            <h1 className="text-2xl font-bold tracking-tight">Siggy Secure Access</h1>
          </div>
          <form action={loginAction} className="space-y-5">
            <input name="user" type="text" placeholder="Username" className="w-full rounded-xl border border-white/10 bg-surface/50 px-5 py-3.5 text-sm outline-none" required />
            <input name="pass" type="password" placeholder="Password" className="w-full rounded-xl border border-white/10 bg-surface/50 px-5 py-3.5 text-sm outline-none" required />
            <button type="submit" className="w-full rounded-xl bg-gradient-to-r from-accent to-yellow-400 py-4 font-mono text-xs font-bold uppercase tracking-widest text-black transition-all">Unlock Dashboard</button>
          </form>
        </div>
      </div>
    );
  ...
  const creators = Array.from(new Set(data.allInvoices.map(inv => inv.creator.username))).sort();
  ...
  const buildUrl = (updates: any) => {
    ...
    Object.entries(updates).forEach(([k, v]) => {
      if (v === null || v === undefined || v === 'all' || v === '') params.delete(k);
      ...
      <div className="relative z-[100] flex min-h-screen bg-[#0a0a0a] text-text-primary font-sans">
        <aside className="fixed left-0 top-0 hidden h-full w-72 flex-col border-r border-white/5 bg-[#0d0d0d] p-6 lg:flex z-[1000]">
          <div className="mb-10 flex items-center gap-3 px-2">
            ...
            <h2 className="text-xl font-bold tracking-tight">Siggy Control</h2>
          </div>
          ...
          <div className="mt-auto p-4 rounded-2xl bg-surface/30 border border-white/5 mb-6">
            <div className="flex items-center gap-2 mb-3">
               <Wallet className="h-4 w-4 text-accent" />
               <p className="text-xs font-bold uppercase tracking-widest text-text-secondary">Payment Info</p>
            </div>
            <p className="text-[10px] text-text-primary leading-relaxed opacity-80">
              Transfer to:<br/>
              <span className="font-bold text-accent">BCA 123456789</span><br/>
              a/n Siggy Admin<br/>
              <span className="mt-2 block opacity-60 italic">Send proof to Discord bot after payment.</span>
            </p>
          </div>
          ...
          <div className="p-8">
            ...
            <div className="mb-10 space-y-6">
              <div className="flex flex-wrap gap-3">
                ...
                {data.guilds.map(g => (
                  <Link key={g.id} href={buildUrl({guild: g.id})} className={`px-5 py-2 rounded-2xl border text-sm font-bold transition-all ${selectedGuild === g.id ? 'border-accent bg-accent/10 text-accent' : 'border-white/5 bg-surface/40 hover:border-white/10'}`}>{g.id.substring(0, 8)}</Link>
                ))}
              </div>
              ...
              {activeTab === 'logs' && (
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 bg-surface/20 p-4 rounded-3xl border border-white/5 backdrop-blur-md">
                   <div className="flex items-center gap-3 px-4 py-2 bg-black/20 rounded-xl border border-white/5">
                     ...
                     <input 
                       type="text" 
                       placeholder="Search Cindy, title..." 
                       className="bg-transparent text-sm outline-none w-full"
                       defaultValue={filters.q}
                       onKeyDown={(e: any) => { if (e.key === 'Enter') window.location.href = buildUrl({q: e.target.value}); }}
                     />
                   </div>
                   <select 
                     className="bg-black/20 text-sm outline-none px-4 py-2 rounded-xl border border-white/5"
                     defaultValue={filters.creator || 'all'}
                     onChange={(e) => window.location.href = buildUrl({creator: e.target.value})}
                   >
              ...
              <>
                ...
                <div className="grid grid-cols-1 gap-10 lg:grid-cols-[1.6fr_1fr]">
                  <div className="space-y-6">
                    <h2 className="text-xl font-bold tracking-tight flex items-center gap-2"><Clock className="h-5 w-5 text-accent" /> Latest Activity</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {recentInvoices.slice(0, 4).map(inv => <DiscordEmbed key={inv.id} invoice={inv} participantAction={markPaidAction} />)}
                    </div>
                  </div>
                  <div className="rounded-3xl border border-white/5 bg-surface/20 p-6 h-fit">
                    <h3 className="text-lg font-bold mb-4 flex items-center gap-2 text-amber-400"><Users className="h-5 w-5" /> Top Debtors</h3>
                    <div className="space-y-3">
                      {data.topDebtors.slice(0, 10).map((d, i) => (
                        <div key={i} className="flex justify-between items-center bg-white/5 p-3 rounded-xl border border-transparent hover:border-white/10 transition-all">
              ...
              <div className="space-y-8">
                <div className="flex items-center justify-between mb-6">
                   <div className="flex items-center gap-3">
                      ...
                      <h2 className="text-2xl font-bold">Debtor Ledger</h2>
                   </div>
                   <div className="bg-surface/30 p-4 rounded-2xl border border-white/5 flex items-center gap-4">
                      <Wallet className="h-5 w-5 text-accent" />
                      <div>
                        <p className="text-[10px] font-bold uppercase tracking-widest text-text-secondary">Payment Guide</p>
                        <p className="text-sm font-bold">BCA 123456789 a/n Siggy Admin</p>
                      </div>
                   </div>
                </div>
                    ...
                    <div key={idx} className="flex flex-col rounded-3xl border border-white/5 bg-surface/30 p-6 shadow-xl transition-all hover:border-amber-400/30">
                      <div className="mb-6 flex items-start justify-between">
                        ...
                        <div className="text-right">
                          <p className="text-[10px] font-mono text-text-secondary uppercase">Sisa Utang</p>
                        ...
                        {debtor.invoices.map((inv, i) => (
                          <div key={i} className="flex items-center justify-between rounded-xl bg-black/20 p-3 border border-transparent hover:border-white/5">
                            <div className="flex flex-col gap-0.5 max-w-[140px]">
                              <span className="text-[11px] font-bold text-text-primary truncate">{inv.title || "Untitled"}</span>
                  ...
                  <div className="space-y-6">
                    {data.monthlyStats.map((m, i) => (
                      <div key={i} className="space-y-2">
                        <div className="flex justify-between text-xs font-mono"><span>{m.label}</span><span>{formatCurrency(m.amount)}</span></div>
                        <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden"><div className="h-full bg-accent shadow-[0_0_10px_rgba(255,215,0,0.4)]" style={{ width: `${(m.amount / Math.max(...data.monthlyStats.map(x => x.amount), 1)) * 100}%` }}></div></div>
                      </div>
                    ))}
                  </div>
                </div>
                <div className="rounded-3xl border border-white/5 bg-surface/30 p-8">
                  <h3 className="text-xl font-bold mb-8">Activity Leaders</h3>
                  <div className="space-y-4">
                    {data.topCreators.map((c, i) => (
                      <div key={i} className="flex justify-between items-center p-4 bg-white/5 rounded-2xl border border-transparent hover:border-white/10 transition-all">
                        <span className="font-bold">{c.name}</span><span className="font-bold text-accent">{formatCurrency(c.totalCreated)}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {activeTab === 'logs' && (
              <div className="space-y-8">
                <div className="flex justify-between items-center">
                  <h2 className="text-2xl font-bold">Management Logs</h2>
                  <p className="text-xs text-text-secondary">{data.filteredInvoices.length} results found</p>
                </div>
                {data.filteredInvoices.length === 0 ? (
                  <div className="py-32 text-center border border-dashed border-white/10 rounded-3xl bg-surface/10"><p className="text-text-secondary">No invoices match your filters.</p></div>
                ) : (
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                    {data.filteredInvoices.map((inv) => <DiscordEmbed key={inv.id} invoice={inv} participantAction={markPaidAction} />)}
                  </div>
                )}
              </div>
            )}
